...
def coffee_net_v2(input_shape, transfer_learning=False):
    """
    ...
    def _coffee_block(input_tensor, n_filters, activation='relu', block_name="1"):
        ...
        _batch_norm1 = tf.keras.layers.BatchNormalization(
            name="batch_norm_1_{}".format(block_name))(_conv1)

        _activation1 = tf.keras.layers.Activation(
            activation="relu_1_{}".format(block_name))(_batch_norm1)
        ...
        _batch_norm2 = tf.keras.layers.BatchNormalization(
            name="batch_norm_2_{}".format(block_name))(_conv2)

        _activation2 = tf.keras.layers.Activation(
            activation="relu_2_{}".format(block_name))(_batch_norm2)

        _max_pool = tf.keras.layers.MaxPool2D(
            pool_size=(2, 2), name="max_pool_{}".format(block_name))(_activation2)
